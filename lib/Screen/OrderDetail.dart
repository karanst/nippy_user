...
/*  returnable() {
  ...
  deliveryBoyDetails(){
    return widget.model!.itemList![0].deliveryBoyName == ""
        ? SizedBox.shrink()
        : Card(
        elevation: 0,
        child: Padding(
            ...
            Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Padding(
                  padding: EdgeInsetsDirectional.only(start: 15.0, end: 15.0),
                  child: Text(
                    widget.model!.itemList![0].deliveryBoyName.toString() + ",",
                  )),
              Padding(
                  padding: EdgeInsetsDirectional.only(start: 15.0, end: 15.0),
                  child: InkWell(
                    onTap: (){
                      _launchCaller(widget.model!.itemList![0].deliveryBoyNumber);
                    },
                    child: Text(widget.model!.itemList![0].deliveryBoyNumber.toString(),
                        style: TextStyle(
                          // color: Theme.of(context).colorScheme.lightBlack2,
...
*/
                  ...
                  children: [
                    ...
                    if(orderItem.status == DELIVERD)
                      Expanded(
                        child: OutlinedButton.icon(
                          onPressed: () {
                            openDriverBottomSheet(context, orderItem.deliveryBoyId);
                          },
                          icon: Icon(Icons.rate_review_outlined,color: colors.primary),
                          label: Text(
                            "Driver Review",
  ...
  setSnackbar(String msg) {
    final scaffoldMessenger = ScaffoldMessenger.of(scaffoldMessengerKey.currentContext!);
            ...
            child: MaterialButton(
              ...
              onPressed: () {
                if (curRating != 0 ||
                    ...
                    (reviewPhotos.isNotEmpty)) {
                  Navigator.pop(context);
                  setDriverRating(curRating, commentTextController.text, driverId);
                } else {
                  Navigator.pop(context);
                  setSnackbar(getTranslated(context, 'REVIEW_W')!);
                }
  ...
  Future<void> setDriverRating(
    ...
    if (_isNetworkAvail) {
      try {
        var request = http.MultipartRequest("POST", setDriverRatingApi);
        request.headers.addAll(headers);
        request.fields[USER_ID] = CUR_USERID!;
